...
const customFilters = {
  ...
  size_button: (
    ...
    const selectedClass = selected ? "selected" : "";
    return `<button class="size-btn ${disabledClass} ${selectedClass}" ${
      available ? "" : "disabled"
    }>${size}</button>`;
  },
  ...
  breadcrumb: (items: Array<{ name: string; url?: string }>) => {
    return items
      .map((item, index) => {
        ...
        return `<a href="${item.url || "#"}" class="breadcrumb-item">${
          item.name
        }</a>`;
      })
...
export const templates = new Environment({
  ...
  debugOptions: {
    position: "bottom", // 'bottom' | 'right' | 'popup'
    height: 300, // per bottom dock
    width: 400, // per right dock
    open: false, // inizia aperto?
